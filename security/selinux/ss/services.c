				    ...
				    const char *reason)
{
	...
	if (context_struct_to_string(scontext,
				     &scontext_name, &length, 0) < 0)
		goto out;

	if (context_struct_to_string(tcontext,
				     &tcontext_name, &length, 0) < 0)
		goto out;
...
#ifdef CONFIG_AUDIT
	if (!context_struct_to_string(context, &s, &len, 0)) {
...
bad:
	...
	rc = context_struct_to_string(&oldc, &s, &len, 0);
	if (rc)
		return rc;
	context_destroy(&oldc);
	context_destroy(c);
...
int security_sid_mls_copy(u32 sid, u32 mls_sid, u32 *new_sid)
{
	...
	if (!policydb_context_isvalid(&policydb, &newcon)) {
		...
		if (rc) {
#ifdef CONFIG_AUDIT
			if (!context_struct_to_string(&newcon, &s, &len, 0)) {
				audit_log(current->audit_context,
					  GFP_ATOMIC, AUDIT_SELINUX_ERR,
