...
static int erofs_cpu_hotplug_notify(struct notifier_block *nb,
				    unsigned long action, void *hcpu)
{
	...
	case CPU_DOWN_FAILED:
		ret = erofs_cpu_online(cpu);
		return ret ? notifier_from_errno(ret) : NOTIFY_OK;
	case CPU_DOWN_PREPARE:
		ret = erofs_cpu_offline(cpu);
		return ret ? notifier_from_errno(ret) : NOTIFY_OK;
	default:
		return NOTIFY_OK;
	}
...
static struct notifier_block erofs_cpu_hotplug_nb = {
	.notifier_call = erofs_cpu_hotplug_notify,
};

static int erofs_cpu_hotplug_init(void)
{
	register_hotcpu_notifier(&erofs_cpu_hotplug_nb);
	return 0;
}

static void erofs_cpu_hotplug_destroy(void)
{
	unregister_hotcpu_notifier(&erofs_cpu_hotplug_nb);
}
